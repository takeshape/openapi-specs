...
function mergeInlineExtensions(spec) {
	...
	for (const [componentName, props] of mergedProps) {
		...
		if (component.allOf && Array.isArray(component.allOf)) {
			...
			const targetItem = component.allOf.find((item) => !item.$ref && item.properties);
			if (targetItem) {
...
function addVariantsProperty(schema) {
	...
	if (schema.allOf) {
		...
		const propsItem = schema.allOf.find((item) => item.properties && !item.$ref);
		if (propsItem) {
